...
public class CameraController : MonoBehaviour
{
    ...
    private Vector3 targetGravityDirection = Vector3.down; // Default gravity
    private bool aligningToGravity = false;
    ...
    void HandleCameraRotationInput()
    {
        ...
        float currentYRotation = Mathf.LerpAngle(
    ...
    void UpdateCameraPosition()
    {
        ...
        transform.position =
            Vector3.Lerp(transform.position, targetPosition, followPlayerSpeed * Time.deltaTime)
    ...
    void ApplyCameraTilt()
    {
        Vector3 playerVelocity =
            new(playerRigidbody.linearVelocity.x, 0, playerRigidbody.linearVelocity.z);
        ...
        if (speed > 1f)
        {
            ...
            float rollAngle =
                -rightDot * maxTiltAngle * Mathf.Min(1.0f, speed * velocityTiltFactor);

            float forwardDot = Vector3.Dot(playerVelocity.normalized, transform.forward);
            float pitchAngle =
                forwardDot * maxTiltAngle * 0.5f * Mathf.Min(1.0f, speed * velocityTiltFactor);
    ...
    public void AlignWithGravity(Vector3 newGravityDirection)
    {
        targetGravityDirection = newGravityDirection;
        aligningToGravity = true;
    }

    void Update()
    {
        if (aligningToGravity)
        {
            ...
            // Stop aligning when close enough
            if (Quaternion.Angle(transform.rotation, targetRotation) < 0.1f)
            {
                transform.rotation = targetRotation;
                aligningToGravity = false;
            }
        }
    }
}
